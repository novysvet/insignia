...
INSIGNIA_GLM53_ALT_SHARD_DIR=/stripe \
  ...
  /var/lib/insignia/glm53-flash-text-striped.index 2>&1 | grep -E 'accepted/round|histogram'
echo ===BATCH===
INSIGNIA_GLM53_ALT_SHARD_DIR=/stripe INSIGNIA_GLM53_DF_BATCH_VERIFY=1 \
  bash /mnt/e/coding/Insignia/build/bench-df.sh "${1:-100}" "${2:-5120}" 4 DUMMY=1 \
  /var/lib/insignia/glm53-flash-text-striped.index 2>&1 | grep -E 'accepted/round|histogram'
