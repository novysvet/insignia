...
DecodeWorkspace::DecodeWorkspace(int ctx,cudaStream_t s):max_context(ctx),stream(s){alloc(&hidden,4096);alloc(&norm,4096);alloc(&qkv,8192);alloc(&attn_gate,4096);alloc(&key,1024);alloc(&value,1024);alloc(&z,4096);alloc(&a,32);alloc(&b,32);alloc(&core,4096);alloc(&gate,12288);alloc(&up,12288);alloc(&down,4096);alloc(&logits,248320);alloc(&delta_state,24*32*128*128);alloc(&conv_state,24*8192*3);alloc(&kv_keys,size_t(8)*ctx*1024);alloc(&kv_values,size_t(8)*ctx*1024);alloc(&mtp_keys,size_t(ctx)*1024);alloc(&mtp_values,size_t(ctx)*1024);cudaMemsetAsync(delta_state,0,24*32*128*128*4,stream);cudaMemsetAsync(conv_state,0,24*8192*3*4,stream);}
...
void Qwen35Decode::linear(const std::string&base,const float*in,float*out){auto m=w_.matrix(base);mxfp4_gemv_mlx((const uint32_t*)m.weight.data,(const uint8_t*)m.scales.data,in,out,m.rows,m.cols,2,x_.stream);w_.release(base);}
...
void Qwen35Decode::attention_layer(int l){if(l<0||l>=32||!Qwen35Shape::full_attention(l))throw std::runtime_error("not a full attention layer");if(x_.position>=x_.max_context)throw std::runtime_error("KV cache full");const std::string p="language_model.model.layers."+std::to_string(l),a=p+".self_attn";auto inw=tensor(p+".input_layernorm.weight");rmsnorm_bf16(x_.hidden,(const uint16_t*)inw.data,x_.norm,1,4096,false,x_.stream);w_.storage().release(p+".input_layernorm.weight");linear(a+".q_proj",x_.norm,x_.gate);split_q_gate(x_.gate,x_.qkv,x_.attn_gate,x_.stream);linear(a+".k_proj",x_.norm,x_.key);linear(a+".v_proj",x_.norm,x_.value);auto qw=tensor(a+".q_norm.weight"),kw=tensor(a+".k_norm.weight");qwen35_qk_norm_rope_gate(x_.qkv,x_.key,(const uint16_t*)qw.data,(const uint16_t*)kw.data,x_.attn_gate,x_.position,x_.stream);w_.storage().release(a+".q_norm.weight");w_.storage().release(a+".k_norm.weight");const int ai=l/4;float*kc=x_.kv_keys+size_t(ai)*x_.max_context*1024,*vc=x_.kv_values+size_t(ai)*x_.max_context*1024;store_kv(x_.key,x_.value,kc,vc,x_.position,x_.stream);gqa_decode(x_.qkv,kc,vc,x_.core,x_.position+1,x_.stream);expand_gate_heads(x_.attn_gate,x_.qkv,x_.stream);sigmoid_mul(x_.core,x_.qkv,4096,x_.stream);linear(a+".o_proj",x_.core,x_.down);residual_add(x_.hidden,x_.down,4096,x_.stream);auto post=tensor(p+".post_attention_layernorm.weight");rmsnorm_bf16(x_.hidden,(const uint16_t*)post.data,x_.norm,1,4096,false,x_.stream);w_.storage().release(p+".post_attention_layernorm.weight");linear(p+".mlp.gate_proj",x_.norm,x_.gate);linear(p+".mlp.up_proj",x_.norm,x_.up);silu_mul(x_.gate,x_.up,x_.gate,12288,x_.stream);linear(p+".mlp.down_proj",x_.gate,x_.down);residual_add(x_.hidden,x_.down,4096,x_.stream);}
void Qwen35Decode::layer(int l){if(Qwen35Shape::full_attention(l))attention_layer(l);else delta_layer(l);}
void Qwen35Decode::forward_token(int token){w_.embed(token,x_.hidden);for(int l=0;l<32;l++)layer(l);auto nw=tensor("language_model.model.norm.weight");rmsnorm_bf16(x_.hidden,(const uint16_t*)nw.data,x_.norm,1,4096,false,x_.stream);w_.storage().release("language_model.model.norm.weight");linear("language_model.lm_head",x_.norm,x_.logits);x_.position++;}
int Qwen35Decode::logits_argmax(){int*dt=nullptr;cudaMalloc(&dt,sizeof(int));argmax_logits(x_.logits,248320,dt,x_.stream);int result=-1;cudaMemcpyAsync(&result,dt,sizeof(int),cudaMemcpyDeviceToHost,x_.stream);cudaStreamSynchronize(x_.stream);cudaFree(dt);return result;}
...
int Qwen35Decode::mtp_draft(int token) {
    ...
    qwen35_qk_norm_rope_gate(x_.qkv, x_.key, (const uint16_t*) qw.data, (const uint16_t*) kw.data, x_.attn_gate, x_.position - 1, x_.stream);
    w_.storage().release(a + ".q_norm.weight");
    w_.storage().release(a + ".k_norm.weight");
    store_kv(x_.key, x_.value, x_.mtp_keys, x_.mtp_values, x_.position - 1, x_.stream);
    gqa_decode(x_.qkv, x_.mtp_keys, x_.mtp_values, x_.core, x_.position, x_.stream);
