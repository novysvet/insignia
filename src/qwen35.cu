...
#include <stdexcept>
namespace insignia {
Qwen35Weights::Qwen35Weights(const ModelFile&m,uint64_t b,cudaStream_t s):storage_(m,b,s),stream_(s){}
Qwen35Weights::~Qwen35Weights()=default;
...
void Qwen35Weights::embed(int token,float*out){if(token<0||token>=Qwen35Shape::vocab)throw std::runtime_error("token out of range");const std::string base="language_model.model.embed_tokens";auto m=matrix(base);mxfp4_get_row_mlx(static_cast<const uint32_t*>(m.weight.data),static_cast<const uint8_t*>(m.scales.data),out,token,Qwen35Shape::hidden,stream_);release(base);}
}
