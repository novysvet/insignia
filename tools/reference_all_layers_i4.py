...
def get(k):
 v=h[k];f.seek(start+v['data_offsets'][0]);raw=f.read(v['data_offsets'][1]-v['data_offsets'][0]);dt={'U32':'<u4','U8':'u1','BF16':'<u2','F32':'<f4','F16':'<f2'}[v['dtype']];a=np.frombuffer(raw,dtype=dt).reshape(v['shape']);return (a.astype(np.uint32)<<16).view(np.float32) if v['dtype']=='BF16' else a
def dq(base):
 w=get(base+'.weight');s=get(base+'.scales');rows=w.shape[0];cols=w.shape[1]*8;u=w.reshape(rows,-1);q=((u[:,:,None]>>(np.arange(8,dtype=np.uint32)*4))&15).reshape(rows,cols);lut=np.array([0,.5,1,1.5,2,3,4,6,-0.,-.5,-1,-1.5,-2,-3,-4,-6],np.float32);sc=np.repeat(s.astype(np.float32),64,axis=1);return lut[q]*sc
def dq_mxfp4(base):
