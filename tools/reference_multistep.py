...
dtmap = {'U32': '<u4', 'U8': 'u1', 'BF16': '<u2', 'F32': '<f4'}
def get(k):
...
def dq(base, sl=None):
    ...
    return LUT[q] * np.repeat(scale, 32, axis=1)
...
def delta(x, l, step):
    ...
    q = q / (np.sqrt(np.mean(q * q, 1, keepdims=True) + 1e-6) * 128)
    k = k / (np.sqrt(np.mean(k * k, 1, keepdims=True) + 1e-6) * np.sqrt(128))
