...
QuantMatrix Qwen35Weights::matrix(const std::string&base){auto w=storage_.acquire(base+".weight"),s=storage_.acquire(base+".scales");if(w.dtype!=DType::u32||!w.shape||w.shape->size()!=2)throw std::runtime_error("not an MXFP4-family matrix: "+base);const int rows=int((*w.shape)[0]),cols=int((*w.shape)[1])*8;const bool i4=s.dtype==DType::f16;
